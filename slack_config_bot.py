...
Deployed on Railway/Render/Fly.io. Commit this file to your GitHub repo root.
...
# ==============================================================================
#   CONFIGURATION  ——  Set these in Railway/Render/Fly Variables tab
#
...
def format_config(cfg: dict) -> str:
    ...
    return (
        ...
        f"*Leads file:*         `{cfg.get('csv_file_path', '?')}`\n"
        f"*Sender Accounts & Counts:*\n{counts_lines}\n"
        f"{'─' * 40}\n"
...
def handle_config_command(text: str, say) -> None:
    ...
    if sub == "set":
        ...
        if key_alias not in SETTABLE_KEYS:
            keys_list = " | ".join(f"`{k}`" for k in SETTABLE_KEYS)
            say(f"❌ Unknown key `{key_alias}`. Available: {keys_list}")
            return
    ...
    say(
        ...
        "  `config set leads_file leads_batch2.xlsx`\n"
        "  `config reset counts`"
    )
